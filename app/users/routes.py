...
def post_register():
    try:
        if request.form.get("password") != request.form.get("password_confirmation"):
            ...
            return render_template(
                "users/register.html", error="The email address is already registered."
            )
        ...
        error = (
            error_message
            or "An error occured while creating an user. Please make sure to enter valid data."
        )
...
def post_login():
    try:
        ...
        error = (
            error_message
            or "An error occured while logging in. Please verify your email and password"
        )
